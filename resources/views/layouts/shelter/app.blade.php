...
<div class="sh-main"><header class="sh-topbar"><button type="button" class="sh-menu" onclick="toggleShelterSidebar()" aria-label="Toggle navigation" aria-controls="shelterSidebar" aria-expanded="true">@include('shelter.partials.icon',['name'=>'menu'])</button><strong>@yield('title','Shelter Profile')</strong><div class="sh-top-search" onclick="window.dispatchEvent(new Event('toggle-command-palette'))" style="cursor:pointer;display:flex;align-items:center;gap:10px;background:#f5f8fb;border:1px solid #dde7f2;border-radius:10px;height:42px;flex:1;max-width:400px;margin:0 16px;padding:0 14px;color:#7087ae;"><i class="bi bi-search" style="font-size:18px;"></i><input type="text" readonly placeholder="Search pages... (Ctrl+K)" style="border:0;background:none;outline:0;width:100%;font-size:13px;color:#1f2937;cursor:pointer;"><kbd style="background:#fff;border:1px solid #dde7f2;color:#7087ae;font-size:11px;padding:2px 6px;border-radius:4px;white-space:nowrap;">Ctrl+K</kbd></div><div class="sh-topbar-right"><a href="{{ route('shelter.notifications.index') }}" class="sh-bell" aria-label="Notifications, {{ $unreadCount }} unread">@include('shelter.partials.icon',['name'=>'notifications'])@if($unreadCount)<span>{{ $unreadCount }}</span>@endif</a><div class="dropdown"><button class="sh-account" type="button" data-bs-toggle="dropdown" aria-expanded="false"><span class="sh-shelter-avatar">@if(Auth::user()->profile_image)<img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url(Auth::user()->profile_image) }}" alt="">@else 🏡 @endif</span><span>{{ $shelterName }}</span><i class="bi bi-chevron-down"></i></button><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="{{ route('profile.edit') }}">Shelter Profile</a></li><li><a class="dropdown-item" href="{{ route('shelter.reviews.index') }}">My Reviews</a></li><li><form method="POST" action="{{ route('logout') }}">@csrf<button class="dropdown-item">Logout</button></form></li></ul></div></div></header>
