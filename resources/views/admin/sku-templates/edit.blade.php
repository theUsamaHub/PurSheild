...
@section('content')
    <div class="mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h4 mb-0 fw-semibold">{{ __('Edit SKU Template') }}</h2>
    ...
    <div class="row">
        <div class="col-lg-8">
            <form action="{{ route('admin.sku-templates.update', $template->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <x-text-input id="name" name="name" type="text" class="form-control" :value="old('name', $template->name)" required autofocus />
                            <x-input-error :messages="$errors->get('name')" class="mt-1" />
                        </div>

                        <div class="mb-3">
                            <x-input-label for="pattern" :value="__('SKU Pattern')" />
                            <x-text-input id="pattern" name="pattern" type="text" class="form-control font-monospace" :value="old('pattern', $template->pattern)" required />
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <x-text-input id="sort_order" name="sort_order" type="number" class="form-control" :value="old('sort_order', $template->sort_order)" min="0" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card mt-4">
                ...
                <div class="card-body text-center">
                    <div id="live-preview" class="p-3 rounded" style="background:var(--bs-tertiary-bg);min-height:60px;">
                        <code class="fs-5 text-primary">{{ str_replace(
