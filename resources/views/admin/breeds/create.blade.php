...
@section('content')
    <div class="mb-4">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="h4 mb-0 fw-semibold">{{ __('Create Breed') }}</h2>
                ...
                <div class="card-body p-4">
                    <form action="{{ route('admin.breeds.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="species_id" name="species_id" class="form-select" required>
                                ...
                                @foreach ($speciesList as $species)
                                    <option value="{{ $species->id }}" {{ old('species_id') == $species->id ? 'selected' : '' }}>{{ $species->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <x-primary-button>{{ __('Create Breed') }}</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
