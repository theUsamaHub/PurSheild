...
<style>
    .pc-main.vet-schedule { font-size:13px; min-width:0; background:linear-gradient(120deg,#fbfdff,#fff 65%); font-family:'Roboto',Arial,sans-serif; --vs-ink:#0b1023; --vs-blue:#305795; --vs-muted:#7389ac; --vs-line:#e4ecf8; --vs-green:#006b55; color:var(--vs-blue); }
    .vet-schedule .pc-content { padding:18px 24px 30px; }
    .vet-schedule .pc-topbar { background:#f5f9fe; border-color:var(--vs-line); padding:12px 24px; }
    .vet-schedule .pc-search { max-width:400px; }
    .vet-schedule .pc-search input { border-color:#93a4c5; background:#fff; color:var(--vs-blue); border-radius:10px; padding:9px 60px 9px 42px; font-size:13px; }
    .vet-schedule .pc-search input::placeholder { color:#5e7aac; }
    .vet-schedule .pc-search i { left:16px; font-size:17px; color:var(--vs-blue); }
    .vet-schedule .pc-search kbd { color:#5e7aac; border-color:#dce7f8; }
    .vet-schedule .pc-user-name { color:var(--vs-ink); font-size:14px; }
    .vet-schedule .pc-user-role { color:var(--vs-blue); font-size:12px; margin-top:4px; }
    .vet-schedule .pc-user-avatar { width:38px; height:38px; }
    .vet-schedule .pc-bell { color:var(--vs-blue); font-size:21px; margin-right:12px; }
    ...
    .vs-heading h1 { color:var(--vs-ink); font-size:clamp(23px,1.8vw,28px); font-weight:700; letter-spacing:-.8px; line-height:1.2; margin:0 0 4px; }
    .vs-heading p { font-size:14px; margin:0; line-height:1.5; }
    .vs-button { display:inline-flex; justify-content:center; align-items:center; gap:8px; min-height:38px; padding:7px 14px; border:1px solid #dce7f8; border-radius:8px; background:#fff; color:var(--vs-blue); font:inherit; text-decoration:none; cursor:pointer; }
    ...
    .vs-add { min-height:44px; min-width:190px; font-size:14px; border-radius:11px; }
    .vs-add i { font-size:22px; line-height:1; }
    .vs-stats { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:16px; margin-bottom:20px; }
    .vs-stat { position:relative; display:flex; align-items:center; gap:18px; min-height:92px; padding:14px 18px; border-radius:12px; background:#edfcf8; }
    .vs-stat-blue { background:#edf5ff; }
    .vs-stat-orange { background:#fff8ef; }
    .vs-stat-icon { flex-shrink:0; width:54px; height:54px; border-radius:50%; background:#d0faeb; display:grid; place-items:center; color:#00975e; font-size:28px; }
    .vs-stat-blue .vs-stat-icon { background:#d6eaff; color:#087cff; }
    .vs-stat-orange .vs-stat-icon { background:#ffecd4; color:#ff8800; }
    .vs-stat strong { display:block; font-size:30px; font-weight:700; line-height:1.15; color:var(--vs-ink); }
    .vs-stat-label { font-size:14px; line-height:1.3; margin-top:4px; white-space:nowrap; }
    .vs-stat-chevron { position:absolute; right:16px; top:20px; display:grid; place-items:center; width:30px; height:30px; border:1px solid #d8e6fa; border-radius:50%; background:#ffffff70; }
    ...
    .vs-panel-heading h2 { display:flex; align-items:center; gap:14px; margin:0; color:var(--vs-ink); font-weight:700; font-size:20px; letter-spacing:-.5px; }
    ...
    .vs-week-table { min-width:740px; font-size:13px; }
    .vs-week-table th { padding:8px 14px; line-height:1.4; }
    .vs-week-table td { padding:7px 14px; height:48px; }
    ...
    .vs-week-table small { font-size:12px; color:#5476ac; }
    .vs-week-table strong { display:block; font-size:14px; line-height:1.25; }
    ...
    .vs-week-table .vs-status { font-size:13px; padding:5px 11px; gap:8px; background:#e9fbf5; }
    .vs-week-table .vs-status-unavailable { background:#ffebee; }
    .vs-week-table .vs-status::before { width:8px; height:8px; }
    .vs-slots { display:flex; flex-wrap:wrap; gap:10px 16px; }
    .vs-slot { background:#f0f5fd; color:var(--vs-blue); padding:5px 12px; border-radius:7px; white-space:nowrap; font-size:13px; }
    ...
    .vs-edit { width:36px; height:33px; background:#f2f6ff; font-size:16px; border-color:#d9e6fb; }
    ...
    .vs-filters input,.vs-filters select { min-height:38px; width:100%; border:1px solid #e0e7f2; border-radius:6px; padding:8px 12px; background-color:#fff; color:#243956; font:inherit; font-size:13px; }
    .vs-field input { padding-left:42px; }
    .vs-filters input::placeholder { color:#7b90b1; }
    .vs-date-placeholder { position:absolute; left:42px; top:50%; transform:translateY(-50%); color:#7b90b1; pointer-events:none; background:white; font-size:13px; }
    ...
    .vs-editor-slot { display:flex; align-items:center; justify-content:space-between; gap:10px; padding:9px 0; border-bottom:1px solid var(--vs-line); }
    ...
    @media(min-width:1500px) { .vs-appointments-table { font-size:13px; } .vs-appointments-table td { height:44px; } .vs-appointments-table .vs-pet-avatar { width:32px; height:32px; } }
    @media(max-width:1499px) { .vet-schedule .pc-topbar { padding:14px 24px; } .vet-schedule .pc-search input { padding-top:8px; padding-bottom:8px; font-size:13px; } .vet-schedule .pc-user-avatar { width:40px; height:40px; } .vet-schedule .pc-content { padding-top:16px; } .vs-heading h1 { line-height:1.1; } .vs-heading p { font-size:14px; } }
    ...
    @media(max-width:767px) { .vet-schedule .pc-content { padding:20px 16px; } .vet-schedule .pc-topbar { padding:12px 16px; } .vet-schedule .pc-user > div:not(.pc-user-avatar),.vet-schedule .pc-search kbd { display:none; } .vet-schedule .pc-search input { padding-right:12px; } .vet-schedule .pc-topbar-right { gap:4px; } .vet-schedule .pc-topbar-right > .dropdown { padding-left:10px; } .vet-schedule .pc-bell { margin-right:8px; } .vs-heading { align-items:flex-start; flex-wrap:wrap; } .vs-add { min-width:0; min-height:44px; font-size:15px; } .vs-stats { grid-template-columns:1fr; gap:10px; } .vs-stat { min-height:88px; } .vs-stat strong { font-size:28px; } .vs-stat-label { font-size:13px; } .vs-stat-chevron { display:grid; } .vs-stat-icon { width:54px; height:54px; } .vs-panel { padding:12px; } .vs-panel-heading { flex-wrap:wrap; } .vs-panel-heading h2 { font-size:18px; } .vs-filters { grid-template-columns:1fr 1fr; } .vs-filters .vs-field:first-child { grid-column:1/-1; } .vs-pagination { margin-left:auto; } }
    ...
    .vet-schedule .modal-title { font-size:17px!important; }
    .vet-schedule .modal-content,.vet-schedule .modal-content :is(.form-control,.form-select,.form-label,.btn) { font-size:13px; }
</style>
@endpush
